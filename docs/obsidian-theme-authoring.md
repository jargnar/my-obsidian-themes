...
Not a plugin. Not a snippet pack. Ten **standalone community-style themes**.
...
| Windows XP Luna, Luna Graph, Windows Vista Aero, Lotus Organizer, Hobonichi Techo, Newton MessagePad, Windows 3.1 Cardfile, Palm OS Memo Pad | `body:is(.theme-dark, .theme-light):not(.is-mobile)` | Period-accurate **light clients** even when the user picked Dark. They also set `color-scheme: light`. |
...
| `Luna Graph/theme.css` | 2194 |
| `Windows Vista Aero/theme.css` | 2186 |
| `Lotus Organizer/theme.css` | 2210 |
...
Light-client themes are longer because they add a **notebook paper + settings readability** chapter after the shared geometry. Aqua / Noir stop after editor polish and folder glyphs. Binding decorations: XP/Vista use a top spiral, Luna Graph uses a left spiral, Hobonichi uses a right-edge bookmark ribbon. Organizer, Newton, Cardfile, and Palm keep the paper/LCD feel without replica rings, letter strips, or silk bars in the editor.
...
- **No remote assets.** Community themes must not fetch fonts or images. XP / Vista rings and paper grain are `data:image/svg+xml,…` URLs.
...
8. **NOTEBOOK PAPER, PERIOD WINDOWS, AND SETTINGS READABILITY** — every light-client theme. Light root panes, period binding where it still helps (XP/Vista top spiral, Luna Graph left spiral, Hobonichi bookmark), Settings ink-on-paper, modal window frames. Organizer / Newton / Cardfile / Palm skip replica rings, silk, and letter-strip overlays. Aqua / Noir do not have this chapter.
...
Light-client extras that are not in Aqua / Noir: grain / paper tokens, window-frame chrome, Start-green (XP / Luna Graph use it for the ribbon orb; Vista currently defines unused copies; Organizer / Hobonichi / Newton / Cardfile / Palm reuse it for period home capsules). XP / Vista / Luna Graph keep `--suite-spiral*` binding art; Hobonichi uses `--suite-bookmark` instead; Organizer, Newton, Cardfile, and Palm do not. Luna Graph, Hobonichi, Newton, Cardfile, and Palm also define `--suite-paper-lines`.
...
| Light graph paper + blue chrome | Luna Graph | same | Same as Luna, with Hobonichi grid lines and a left spiral instead of a top spiral. No bookmark ribbon. |
| Light notepad + glass chrome | Windows Vista Aero | same | Same as Luna. Do not assume Vista's `--suite-start-green` does anything. |
| Light notepad + burgundy leather | Lotus Organizer | same | Same as Luna, minus the spiral overlay. Rainbow section-tab folders stay in the file explorer. No replica binder rings. |
